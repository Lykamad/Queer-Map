...
//Si no lo ponemos a observar no actuliza la vista cuando añade un nuevo usuario
    val userViewModelList : List<LocationsEntity> by userViewModel.readAllData.observeAsState(listOf())

    NavHost(navController = navController, startDestination = "listarUsuarios"){
        composable("viewLocations"){ ListUsers(userViewModel) }
        composable("createLocations"){ ProfileEdit(navController, userViewModel, userViewModelList) }} //el orden importa //en el 3r val espera UserEntity pero le paso User -> ERROR
}